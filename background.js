...
function updateBlocking(blockedSites) {
  ...
  if (blockedSites.length > 0) {
      ...
      // Unregister existing content scripts to avoid duplicates
      chrome.scripting.unregisterContentScripts(null, () => {
          // Register new content scripts for the blocked domains
          chrome.scripting.registerContentScripts([{
              id: 'blocker',
              matches: urls,
              js: ['content.js'],
...
// Load the initial list of blocked sites and set up the listener
chrome.storage.local.get({ blockedSites: ['jinxcode.ng', 'henry.ng', 'google.com'] }, function (result) {
  updateBlocking(result.blockedSites);
});
